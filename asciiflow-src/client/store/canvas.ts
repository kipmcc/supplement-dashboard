...
export class CanvasStore {
  ...
  /**
   * Load ASCII text directly into the canvas, replacing current content.
   * Used by AviFlow bridge for Supabase-backed diagrams.
   */
  loadFromText(text: string) {
    const layer = textToLayer(text, new Vector(0, 0));
    this.persistentCommitted.set(layer);
    this.undoLayers.set([]);
    this.redoLayers.set([]);
